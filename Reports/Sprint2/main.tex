...
\begin{titlepage}
    ...
    \vspace{1.5cm}
    {\Huge\bfseries Sprint 2: Project Progress\par}
    \vspace{1cm}
    ...
    \vspace{0.5cm}
    {\large \textbf{Normative and Dynamic Virtual Worlds}\\ Sprint 2: Project Progress\par}
    \vspace{1cm}
    {\large\bfseries November 17th, 2025\par}
\end{titlepage}
...
See Figure~\ref{fig:game_flow} for a schematic representation of this structure.
...
Of course, the aim of this course is not to build games, but to explore the use of AI techniques in virtual worlds.
The main AI components of our game are:
\begin{itemize}
    \item \textbf{Procedural Content Generation (PCG):} We will implement a BSP-based dungeon generator to create unique layouts for each run. 
      This will involve algorithmic generation of rooms, corridors, and enemy placements.
      We will also explore cellular automata techniques for room interior design.
    \item \textbf{Generative AI for Narrative and Music:} 
      We will use vision-language models to analyze room screenshots and generate contextually relevant narrative elements (NPCs, quests, lore) using large language models (LLMs). 
      Additionally, we will employ generative music models to create adaptive soundtracks that reflect the mood of each room.
    \item \textbf{Reinforcement Learning (RL) for Boss Behavior:} 
      The three main bosses will be controlled by RL agents trained through self-play. 
      These agents will learn to adapt their strategies based on player actions, resulting in challenging encounters.
\end{itemize}
...
\subsubsection{BSP Map Layout Generation}
...
% \subsubsection{Cellular Automata Room Generation}
% \label{subsubsec:ca_rooms}

% [Placeholder for Dani's work]
...
The pipeline operates sequentially: a vision model first analyzes a screenshot of the generated room to produce a semantic description of the environment (Section~\ref{subsubsec:vision_analysis}). This description is then fed to a large language model (LLM) that generates contextual narrative content including non-player characters (NPCs), quests, and lore entries (Section~\ref{subsubsec:narrative_gen}). 
Finally, the emotional tone extracted from the vision analysis drives procedural music generation to match the room's atmosphere (Section~\ref{subsubsec:music_gen}).
This vision-driven approach should make it so that narrative and audio content semantically align with the visual characteristics of each room, rather than being generated independently.

\subsubsection{System Architecture}
\label{subsubsec:genai_architecture}

We implement all three generative AI components as a unified Python backend service, architecturally decoupled from the Unity game engine. 
This design decision addresses several fundamental constraints inherent to deploying deep learning models within game engines.

Modern generative AI models -- particularly transformer-based architectures for vision, language, and audio -- rely on the Python ecosystem for inference. 
Critical dependencies include PyTorch for tensor operations, HuggingFace Transformers for model loading and execution, and specialized inference engines such as Ollama for optimized LLM serving. 
These libraries are not available in Unity's C\# runtime environment, and reimplementing GPU-accelerated inference from scratch would be prohibitively complex. 
By extracting AI inference into a separate Python process, we gain  access to state-of-the-art models with well-tested implementations.

The backend is structured as a FastAPI~\cite{fastapi} web service exposing RESTful HTTP endpoints. Each generative component corresponds to a dedicated endpoint:

\begin{lstlisting}[language=bash, frame=single, basicstyle=\ttfamily\footnotesize, floatplacement=H]
...
Table~\ref{tab:ai_models} summarizes the three AI models employed in our pipeline and their computational requirements. All models were selected based on their open-source availability and lightweightness, ensuring they could run efficiently on consumer-grade hardware (RTX 5070 Ti, 16GB VRAM). The vision and music models were chosen after evaluating multiple alternatives to find the best balance between output quality and inference speed for real-time game integration.
...
\begin{tabular}{lcc}
\hline
\textbf{Component} & \textbf{Model} & \textbf{Parameters} \\
\hline
Vision Analysis & moondream2~\cite{moondream2024} & 1.86B \\
Narrative & llama2~\cite{touvron2023llama} (via Ollama) & 7B \\
Music & MusicGen Medium~\cite{copet2023musicgen} & 1.5B \\
\hline
\end{tabular}
\end{table}

Figure~\ref{fig:genai_architecture} illustrates the complete system architecture showing the sequential pipeline from room generation through vision analysis, narrative generation, and music synthesis.

\begin{figure}[H]
  \centering
  \begin{lstlisting}[basicstyle=\ttfamily\scriptsize, frame=single]
+---------------------+     +----------------------+     +-----------------+
|   UNITY CLIENT      |     |  FASTAPI BACKEND     |     |   AI MODELS     |
|                     |     |                      |     |                 |
|  +--------------+   |     |  +---------------+   |     |  +-----------+  |
|  |BSP Generator |   |     |  | POST          |-------->|  |moondream2 |  |
|  +------+-------+   |     |  | /analyze/     |   |     |  +-----+-----+  |
|         |           |     |  | vision        |   |     |        |        |
|         v           |     |  +-------+-------+   |     |        |        |
|  +--------------+   |     |          |           |     |        v        |
...
+---------------------+     +----------------------+     +-----------------+
  \end{lstlisting}
  \caption{Generative AI pipeline architecture showing the sequential flow from Unity client through FastAPI backend to AI models, with caching layer.}
...
For development and testing, the generative AI pipeline was validated using the three tutorial rooms from the base game (Section~\ref{sec:tutorial}) rather than procedurally generated dungeons -- that is left for Sprint 3. 
These manually designed rooms provided stable, reproducible test cases to evaluate the vision analysis quality, narrative coherence, and music generation effectiveness before integrating with the BSP dungeon generator. Future work will connect the pipeline to procedurally generated room layouts.

\subsubsection{Vision-Based Room Analysis}
\label{subsubsec:vision_analysis}
...
\begin{lstlisting}[language=json, basicstyle=\ttfamily\scriptsize, frame=single]
{
  "environment": "A grassy clearing with scattered ponds and small stone
                  structures. The area has a peaceful yet mysterious
                  atmosphere.",
  "npc": {
...
Example generated music tracks for different game contexts~\footnote{To listen to these, please open with Adobe Acroabt; in case it is not possible, we also attached the songs in the delivery.}:

\begin{itemize}
  \item \textbf{tavern.mp3}: Prompt ``tavern'' - peaceful, ambient music for safe areas \\
  \textattachfile[color=0 0 1, icon=Speaker, description={Tavern ambient music}]{figures/tavern.mp3}{\textcolor{blue}{Play/Download Audio}}

  \item \textbf{mysterious.mp3}: Prompt ``mysterious'' - atmospheric exploration music \\
  \textattachfile[color=0 0 1, icon=Speaker, description={Mysterious exploration music}]{figures/mysterious.mp3}{\textcolor{blue}{Play/Download Audio}}

  \item \textbf{boss\_tense.mp3}: Prompt ``boss tense'' - intense music for boss encounters \\
  \textattachfile[color=0 0 1, icon=Speaker, description={Boss battle music}]{figures/boss_tense.mp3}{\textcolor{blue}{Play/Download Audio}}
\end{itemize}
...
\paragraph{Future Integration and Potential Extensions}
The generative AI pipeline components (vision-based room analysis, narrative generation, and music generation) have been tested individually and show promising potential for procedural content generation. 
However, for Sprint 3 substantial work remains to integrate these systems with the procedural dungeon generation once the BSP room layouts are finalized. Integration tasks include refining prompts based on actual dungeon characteristics, implementing in-game logic for content delivery, and optimizing the generation flow for runtime performance.

Additionally, we are exploring the possibility of incorporating voice generation models to create a narrative experience. 
For instance, generated story segments could be converted to speech using text-to-speech models, creating a narrator that reads the lore and dialogue to players. This remains an experimental direction that has not yet been tested, and feasibility will depend on available time and technical constraints.
