...
Sir Cael is trapped in a magic book that rewrites the dungeon every time he dies.
The dungeon has three chapters, each ending with a boss.

If Cael beats boss 1 or boss 2, that chapter becomes fixed and will not change anymore.
If Cael dies later, only the current chapter is regenerated.

In the last chapter, Cael fights the Chronicler, the one causing the loop.
If Cael wins, the rewriting stops and the story ends.

\subsubsection{Spoilers!}
Cael is the Chronicler. He created the book long ago to rewrite history until it was ``perfect'', and got stuck in his own loop.
Boss 1 and boss 2 are obstacles created by his fear and denial.
The final boss is Cael confronting the part of himself that refuses to stop rewriting.
