...
Figure~\ref{fig:software_architecture} shows the overall software architecture of our system, illustrating how these AI-driven components interact with the Unity game engine and with each other.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/SoftwareArchitectureDiagram.png}
    \caption{Software architecture diagram showing the integration of PCG, GenAI, and RL components within the Unity game engine. The DungeonRunManager orchestrates chapter progression, while the BSP generator and CA-based room populator create the dungeon structure. The GenAI pipeline generates narrative and music content, and RL-trained agents control boss behaviors.}
...
\begin{itemize}
  ...
  \item \textbf{Dániel Mácsai}: Procedural room population (CA implementation), chapter theming 
...
\section{Cellular Automata Pseudocode}
\label{app:ca_pseudocode}

This appendix provides the complete pseudocode for the Cellular Automata algorithm used for room population (lakes, paths, and decorations).
