...
The player starts with 3 health points by default, represented by a health bar that decreases upon taking damage from enemy attacks. Health can be replenished by collecting hearts, which have a chance to drop from defeated enemies. Additionally, fallen enemies drop coins that can be collected. While not yet implemented, the plan is for these coins to be used as currency in a shop system, should time permit its development.
...
The technical specifics of the training process, reward functions, and agent architecture for these bosses are detailed further in Sections~\ref{sec:rl_proposal} and \ref{sec:rl_methodology_and_results}.
...
\input{sections/rl_methodology_and_results.tex}
...
\begin{itemize}
  ...
  \item \textbf{Dániel Mácsai}: Procedural room population (CA implementation)
  \item \textbf{Jean Dié}: Generative AI pipeline (vision analysis, LLM-driven narrative, and music generation)
